...
def drive_build_service(token_path: Path, cfg: Dict[str, Any] = None) -> Any:
    ...
    cfg: دیکشنری config برای خواندن oauth_client_id و oauth_client_secret
    """
...
def _call_gemini_core(
    ...
    for api_key in ordered:
        ...
        for attempt in range(MAX_RETRIES_PER_KEY):
            ...
            try:
                logging.info(f"API: key {key_num}, attempt {attempt+1}/{MAX_RETRIES_PER_KEY} (model={model})")
        ...
        if net_key_failures >= NETWORK_KEY_CAP:
...
def extract_pdf(
    ...
    def _extract_bytes(pdf_bytes: bytes, worker_id: int, label: str, depth: int = 0) -> List[Dict[str, Any]]:
        """
        ...
        try:
            raw = call_gemini_with_pdf(
                b64, prompt, key_manager, model, last_call_time, cancel_event,
                meta=meta, worker_id=worker_id,
            )
    ...
    if total_pages > 0 and total_pages > EXTRACTION_CHUNK_PAGES:
        ...
        # ── پاسِ دومِ ترتیبی: بخش‌های ناموفق را تک‌به‌تک دوباره امتحان کن (اگر مسدودیتِ منطقه‌ای نبود) ──
        if failed and not _cancelled() and not region_blocked:
            if log_callback:
                log_callback(f"     تلاشِ مجددِ ترتیبی برای {len(failed)} بخشِ ناموفق (تک‌به‌تک، برای اتصالِ کند)…")
            for it in sorted(failed, key=lambda x: x[0]):
                if _cancelled():
                    break
                idx, start, end, pdf_bytes = it
                try:
                    rows_retry = _extract_bytes(pdf_bytes, worker_id=idx, label=f"بخش{idx+1}(retry)")
...
def launch_ui(app_dir: Path) -> None:
    ...
    def _drive_pre(ts: str, all_input_pdfs: List[Path]):
        ...
        service = drive_build_service(token_path, cfg)     # اولین‌بار مرورگر باز می‌شود
    ...
    def run_auto_mode(api_keys, model_name, inputs, category, manual_features, mothers=None):
        ...
        try:
            ...
            for folder in folders:
                ...
                # خروجی Excel + JSON این فولدر → داخل فولدر واحد اجرا (run_dir)؛ JSON در Drive هم آپلود می‌شود
                if folder_results:
                    fout = run_dir / _unique_out_name(folder.name)
                    create_excel_output(folder_results, features, fout, include_source=True)
                    emit_json_and_upload(folder_results, features, fout, True, service, ts_id, drive_ok)
                    log(f"  💾 ذخیره: {fout.name} (+ JSON)  ({len(folder_results)} ردیف)")
            ...
            if single_pdfs:
                ...
                if features:
                    ...
                    # هر PDF تکی → یک xlsx + JSON داخل فولدر واحد اجرا (run_dir)؛ JSON در Drive هم آپلود می‌شود
                    for sp in single_pdfs:
                        ...
                        if sp_results:
                            ...
                            emit_json_and_upload(sp_results, features, fout, False, service, ts_id, drive_ok)
                            log(f"  💾 ذخیره: {fout.name} (+ JSON)")
                    if single_results:
