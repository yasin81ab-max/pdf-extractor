...
SUBJECTS_URL = "https://raw.githubusercontent.com/yasin81ab-max/pdf-extractor/main/subjects.json"
    ...
    for start in range(0, len(unique_records), IMAGE_DESC_BATCH):
        ...
        for rec in batch:
            try:
                ...
                data = b""
            if not data:
                # فایل ناخوانا/خالی ارسال نمی‌شود تا کل batch خراب نشود
                continue
            images.append((base64.b64encode(data).decode("utf-8"), _mime_for_ext(rec["ext"])))
            sent_recs.append(rec)
        if not sent_recs:
            continue

        prompt = build_image_desc_prompt(len(sent_recs), subjects)
        try:
...
def embed_image_metadata(path: Path, desc: Dict[str, Any]) -> None:
    ...
    try:
        ...
        img = Image.open(path)
        img.load()   # فایل کاملاً خوانده شود تا save روی همان مسیر در ویندوز خطا ندهد
...
def generate_word_and_images(
    ...
    for idx, pdf in enumerate(pdfs, 1):
        ...
        # تصاویر و Word به فولدر موقت می‌روند و فقط در Drive ذخیره می‌شوند
        img_dir = out_dir / "_temp_media" / pdf.stem
        ...
        try:
            ...
            per_path = out_dir / "_temp_media" / f"{pdf.stem}.docx"
    ...
    if DOCX_AVAILABLE and combined_doc is not None and any_text:
        try:
            combined_path = out_dir / "_temp_media" / f"{label}.docx"
...
def launch_ui(app_dir: Path) -> None:
    ...
    tk.Checkbutton(
        ...
        selectcolor="#0c2445", activebackground=C_ROOT, activeforeground=C_WHITE,
        font=("Arial", 9), bd=0, highlightthickness=0,
    ).pack(anchor="e", pady=(8, 0))

    # ════════════════ تب اصلی ════════════════
    # ── موضوع (مولتی‌سلکت از روی subjects.json) ──
    subj_out, subj_body = make_section(main_page, "  موضوع (الزامی) — جستجو و انتخاب چندتایی")
    subj_out.pack(fill="x", pady=(0, 8))

    subject_vars: Dict[str, Any] = {}      # subject(str) → BooleanVar
    selected_summary_var = tk.StringVar(value="موضوعی انتخاب نشده")
    _all_subjects: List[str] = []          # لیست کامل موضوع‌ها
    _popover = {"win": None}               # رفرنس پنجره‌ی باز
    ...
    def _refresh_subject_summary():
        ...
        if not sel:
            selected_summary_var.set("موضوعی انتخاب نشده")
        elif len(sel) <= 3:
            selected_summary_var.set("، ".join(sel))
        else:
            selected_summary_var.set(f"{len(sel)} مورد انتخاب شد: {('، '.join(sel[:2]))} ...")

    def get_selected_subjects() -> List[str]:
        return [s for s, v in subject_vars.items() if v.get()]

    def _populate_subjects(items: List[str]):
        nonlocal _all_subjects
        # فقط انتخاب‌های جلسه‌ی فعلی حفظ می‌شود — هیچ pre-selection از config ذخیره‌شده نیست
        current = {s for s, v in subject_vars.items() if v.get()}
        subject_vars.clear()
        _all_subjects = list(items)
        for s in items:
            subject_vars[s] = tk.BooleanVar(value=(s in current))
        _refresh_subject_summary()
        # اگه پنجره بازه، لیستش رو هم تازه کن
        if _popover["win"] is not None and tk.Toplevel.winfo_exists(_popover["win"]):
            _rebuild_list()

    # ── ساخت پنجره‌ی پاپ‌آور (دراپ‌داون سفارشی) ──
    ...
    def _rebuild_list(*_):
        ...
        for s in _all_subjects:
            if query and query not in s.lower():
                continue
            cb = tk.Checkbutton(
                inner, text=s, variable=subject_vars[s],
    ...
    def _toggle_popover():
        ...
        if not _all_subjects:
            set_status("لیست موضوع خالی است — ابتدا بارگیری کنید")
            return

        win = tk.Toplevel(subj_btn)
        win.overrideredirect(True)   # بدون نوار عنوان
        win.configure(bg=C_BORDER)
        _popover["win"] = win

        # موقعیت زیر دکمه
        subj_btn.update_idletasks()
        x = subj_btn.winfo_rootx()
        y = subj_btn.winfo_rooty() + subj_btn.winfo_height() + 2
        w = max(subj_btn.winfo_width(), 320)
    ...
    subj_btn = tk.Button(
        subj_row, textvariable=selected_summary_var, command=_toggle_popover,
    ...
    subj_btn.pack(side="right", fill="x", expand=True)

    # ── بستن پاپ‌اور با کلیک بیرون از آن ──
    # این binding یک‌بار ست می‌شود و برای تمام عمر پنجره فعال است.
    # overrideredirect(True) رویداد FocusOut را قابل‌اعتماد نمی‌کند؛
    # بنابراین موقعیت کلیک را با مرز پاپ‌اور مقایسه می‌کنیم.
    def _global_btn_press(event):
        if _popover["win"] is None:
            return
        try:
            # کلیک روی خودِ دکمه‌ی dropdown → _toggle_popover کارش را می‌کند
            bx = subj_btn.winfo_rootx()
            by = subj_btn.winfo_rooty()
            bw = subj_btn.winfo_width()
            bh = subj_btn.winfo_height()
            if bx <= event.x_root <= bx + bw and by <= event.y_root <= by + bh:
                return
            # کلیک داخل خود پاپ‌اور → ادامه تعامل مجاز است
            pw = _popover["win"]
    ...
    _run_state: Dict[str, Any] = {
        "active": False, "ts": "", "subjects": [], "total_rows": 0
    }
    ...
    def _post_run_notify(
        ...
        def _cleanup_temp(dirs):
            """حذف _temp_media از فولدرهای خروجی — Word و عکس‌ها فقط در Drive نگه‌داشته می‌شوند."""
        ...
        try:
            ...
            for (in_dir, out_dir_path) in out_dirs:
                ...
                # حذف فایل‌های موقت Word و تصاویر از حافظه‌ی محلی
                temp_media = out_dir_path / "_temp_media"
                if temp_media.exists():
                    try:
                        import shutil
                        shutil.rmtree(temp_media)
                        log(f"     🗑 فایل‌های موقت حذف شد: {temp_media.name}")
                    except Exception as e:
                        log(f"     ⚠ حذف ناموفق: {str(e)[:40]}")
            ...
            # آپلود فایل منابع به Drive
            import tempfile, os as _os
            sources_content = f"زمان اجرا: {ts}\nموضوع‌ها: {', '.join(subjects) if subjects else '—'}\n\nمسیرهای ورودی:\n" + "\n".join(str(p) for p in input_pdfs)
            try:
                tmp_src = Path(tempfile.gettempdir()) / f"manabe_{ts}.txt"
                tmp_src.write_text(sources_content, encoding="utf-8")
                _drive_upload_file(service, tmp_src, ts_id)
                tmp_src.unlink(missing_ok=True)
            except Exception as e:
                log(f"     ⚠ آپلود منابع.txt ناموفق: {str(e)[:50]}")
            ...
            # حتی در صورت خطای Drive، فایل‌های موقت محلی حذف می‌شوند
            _cleanup_temp(out_dirs)
            return False
    ...
    def run_auto_mode(api_keys, model_name, inputs, category, manual_features, subjects=None):
        ...
        _run_state["ts"] = ""
        _run_state["subjects"] = list(subjects)
        ...
        try:
            ...
            for folder in folders:
                ...
                # فایل متنی منابع کنار فولدر خروجی
                _sources_txt = folder_out.parent / f"{folder_out.name}_منابع.txt"
                try:
                    subj_line = "موضوع‌ها: " + "، ".join(subjects) if subjects else "موضوع: —"
                    _sources_txt.write_text(
                        f"زمان اجرا: {ts}\n{subj_line}\n\nمسیرهای ورودی:\n" +
            ...
            if single_pdfs:
                ...
                if features:
                    ...
                    # فولدر ترکیبی Word داخل فولدر ورودی اول (نه کنار PDF‌ها)
                    single_combined_out = single_pdfs[0].parent / "single_pdfs_out"
                    single_combined_out.mkdir(parents=True, exist_ok=True)
                    generate_word_and_images(
                        single_pdfs, single_combined_out, "PDFهای_تکی", ts,
                        key_manager, model_name, last_call_time, cancel_event, log, set_status, subjects=subjects, desc_enabled=_desc_enabled(),
                    )
                    # عکس‌ها/Word توی single_combined_out/_temp_media ذخیره شدن — باید آپلود بشن
                    all_out_dirs.append((single_pdfs[0].parent, single_combined_out))
                else:
                    log("  ⚠ هیچ ویژگی‌ای مشخص نشد — PDFهای تکی رد شد.")

            drive_ok = _post_run_notify(cfg, subjects, ts, all_input_pdfs, all_out_dirs, total_rows, log)

            if total_rows:
                log(f"\n✅ تمام شد — مجموعاً {total_rows} ردیف.")
                set_status(f"✅ تمام — {total_rows} ردیف")
                if last_out_dir and drive_ok:
                    try:
                        import subprocess
            ...
            _tgram_notify(
                cfg, "کاربر", _run_state.get("subjects", []),
        ...
        if _run_state.get("active"):
            def _notify_stop():
                _tgram_notify(
                    cfg, "کاربر", _run_state.get("subjects", []),
                    _run_state.get("ts", "—"), success=False,
                    error_msg="اجرا توسط کاربر متوقف شد",
                )
        ...
        if _run_state.get("active"):
            def _notify_exit():
                _tgram_notify(
                    cfg, "کاربر", _run_state.get("subjects", []),
                    _run_state.get("ts", "—"), success=False,
                    error_msg="کاربر پنجره را بدون اتمام اجرا بست",
                )
